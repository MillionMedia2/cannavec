...
const LIVE_CARDS = [
  ...
  },
  {
    icon: Zap,
    title: "Skills",
    description: "Purpose-built tools for accessing the Cannabis Knowledge Base — regulatory checks, evidence filters, and more.",
...
export default async function DashboardPage() {
  ...
  const visibleCards = LIVE_CARDS.filter((c) => !c.paidOnly || canAccessPaid);

  return (
    <div className="min-h-screen bg-warm-50 flex">
      <DashboardSidebar isAdmin={isAdmin} currentPath="/dashboard" />
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        <main className="flex-1 px-6 py-8 max-w-5xl">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {visibleCards.map(({ icon: Icon, title, description, href, badge }) => (
              <Link
                key={title}
                href={href}
